...
class BookRequest(BaseModel):
    ...
    published_date: int = Field(gt=1997, lt=2050)
...
async def create_book(book_request: BookRequest):
    new_book = Book(**book_request.model_dump()) # convert into Book object instead of Pydantic object
    # `.dict()` is now renamed to `.model_dump()` in Pydantic v2
    BOOKS.append(find_book_id(new_book))


def find_book_id(book: Book): # create ID for a new created book from /create-book
